              ...
              <div class="dropdown">
                  ...
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1" style="width: auto;">
                    <li class="d-flex justify-content-between align-items-center">
                      <router-link :to="`/user/EditUser/${user.IDUSER}`" class="btn btn-sm btn-primary">
                        <img :src="require(`../assets/edit.png`)" style="width: 20px; height: 20px; border-radius: 50%;">
                      </router-link>
                      <button @click="showDeleteConfirmation(user.IDUSER)" class="btn btn-sm btn-danger" style="display: flex; justify-content: center; align-items: center; margin-right: 40px;">
...
export default {
  ...
  methods: {
    ...
    showDeleteConfirmation(Id) {
      ...
      console.log('ID reçu :', Id); // Vérifiez ici si l'ID est correct
    if (!Id) {
      console.error('ID de l\'utilisateur à supprimer est indéfini');
      return;
    }  
        ...
        Swal.fire({
          ...
          axios.delete(`/api/users/deletusers/${Id}`)
          .then(response => {
