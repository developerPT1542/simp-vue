<template>
  ...
  <div class="container-box">
    <h3>PROXIES</h3>
    <div v-for="header in headers" :key="header.proxies_id">
      <label class="checkbox-container">
        <input type="checkbox" v-model="selectedHeaders" :value="header.proxies_id" />
        {{ header.KEY_PARAM }}
      </label>
    </div>
  </div>
    ...
    methods: {
      async fetchUrlsAndHeaders() {
        ...
        try {
          ...
          const response = await axios.get(`/api/users/${endpointName}/urlsandproxies`);
      ...
      async submit() {
        ...
        try {
          ...
          await axios.post('/api/users/addproxiesUrls', data);
          Swal.fire({
            position: 'top-end',
            icon: 'success',
            title: 'proxieUrl a été ajouté',
            showConfirmButton: false,
            timer: 1500
          });
          this.$router.push({ name: 'ProcUrl' });
        } catch (error) {
