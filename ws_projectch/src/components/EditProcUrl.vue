  ...
  export default {
    ...
    methods: {
      async fetchHeaderUrl() {
        ...
        try {
          const response = await axios.get(`/api/users/proxiesUrl/${Id}`);
      ...
      async updateHeaderUrl() {
        try {
            const response = await axios.put(`/api/users/proxiesUrl/${this.editedUser.Id}`, this.editedUser);
          if (response.status === 200) {
            console.log('Mise à jour réussie, redirection...');
            Swal.fire({
              position: 'top-end',
              icon: 'success',
              title: 'proxiesUrl a été modifié',
